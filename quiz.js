...
const winFiles = [
    ...
    { audio: 'social-credit_751J4TV.mp3',          image: null },
    { audio: 'social-credit-music.mp3',            image: null },
];
...
async function loadEntry(entry) {
    try {
        const res = await fetch(BASE + entry.audio);
        const arrayBuf = await res.arrayBuffer();
        const buffer = await audioCtx.decodeAudioData(arrayBuf);
        return { buffer, image: entry.image };
    } catch (e) {
        console.warn('Could not load:', entry.audio, e);
        return null;
    }
}
...
async function preloadAll() {
    const [fails, wins] = await Promise.all([
        Promise.all(failFiles.map(loadEntry)),
        Promise.all(winFiles.map(loadEntry))
    ]);
...
function playBuffer(buffers) {
    ...
    stopCurrent();
    const entry  = buffers[Math.floor(Math.random() * buffers.length)];
